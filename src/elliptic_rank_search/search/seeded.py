...
import tempfile
import time
from elliptic_rank_search.runtime import PACKAGE_ROOT, code_hashes

from elliptic_rank_search.search.search_policy import (GP, ROOT, POLICY, boxes, clean,
                         diverse_vectors, order_models)
from elliptic_rank_search.search.bootstrap import save, checked_script, gp_process
from elliptic_rank_search.search.models import prepare_models, parity_vectors, parity_order, record_coverage, search_script
from elliptic_rank_search.certificates.torsion_certificate import certify
from elliptic_rank_search.search.point_models import divide_basis,prepare_covers,isogenous_sources


# Isolated namespace for the in-memory GP adapter used by this engine.
_pool_spec=importlib.util.spec_from_file_location('equation_anchor_pool',PACKAGE_ROOT/'search/bounded_anchor_pool.py')
...
def independent_result(data, expected):
    path = PACKAGE_ROOT/'certificates/certificate.py'
    ...
    from elliptic_rank_search.certificates.torsion_certificate import torsion_points,select_basis,verify_certificate,singleton_certificate
...
def search(source, output, seconds=300, workers=4, anchors=2048, target=32,
    ...
    if anchor_mode=='unified':
        from elliptic_rank_search.search.unified import search as unified_search
