...
`data/icarm-20260917.json` contains the frozen public catalogue metadata. Historical downloads' digests and per-case source hashes preserve provenance. Old paths in `experiments/history/` identify historical local records; no executable recipe reads those paths. Machine-independent recipes live outside that history directory.
