...
All commands below assume that the current directory is the project root. Python 3.10+ and GP are the only search dependencies. Python certificate replay requires no external program.
...
`run.py` requires an empty output directory inside the project. A nonempty directory is rejected so an equation-only trial cannot silently consume a previous search. The default CLI target is 31; specify a realistic target for the curve under investigation.
...
The command-line data guard permits the declared input, Python code, and output files within this project. Search workers are not given the other example witnesses. This is an audit mechanism, not an operating-system security sandbox; it does not monitor arbitrary reads performed by GP or reads outside the project. GP scripts are generated from validated arithmetic data and checked against a forbidden-call list for rank/database and external-data operations.
...
python verify.py examples/icarm-199/points.json --certificate examples/icarm-199/certificate.json

# Recompute a conservative local-character certificate from arbitrary points:
python certificate.py examples/icarm-199/points.json --output runs/new-certificate.json --max-prime 2000
```
...
Copy this directory to your intended repository location, excluding generated `runs/`, `artifacts/`, and `__pycache__/` directories. Its own `.gitignore`, `.gitattributes`, and MIT license are included. Keep the `examples/` witnesses and attribution with the code. Run the proof replay and tests from the copied directory before publishing it.
